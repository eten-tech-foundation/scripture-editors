...
| `extendCharacterMarker`              | Extends a character marker over the whole selection, wrapping only the parts not already covered so no nested identical markers are produced. Returns `false` and changes nothing when the selection is collapsed or already fully covered; throws if readonly or a marker is unsupported |
