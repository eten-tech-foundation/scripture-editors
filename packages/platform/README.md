...
| `replaceCharacterMarker`             | Changes a character marker, keeping its text. Changes nothing when no marker matches, the marker is already the target, the selection is in a note, or the change can't be confined to the selection; throws if readonly or a marker is unsupported |
