 ...
 * call site and every TYPED-input rebuild): the own-marker-prefix dedup recognizes a PASTE shape
 * (a whole-paragraph copy's own glyph riding along with the pasted text) and must not also apply
 * to a user typing the same byte sequence — see that function's doc comment for why. `$rebuildParas`
 * is the only caller that ever passes `true`, and only when `Tier2Context.pasteRebuildArmed` says
 * the triggering rebuild is inside an external paste's own update.
 */
...
export function $rebuildParas(paras: ParaNode[], context: Tier2Context): boolean {
  ...
  for (const para of paras) {
    const fragment = $buildParaFragment(para, getMarkerFn, isPasteRebuild);
    if (!fragment) {
