...
function $handleCharText(
  ...
  const segment = typeof attributes.segment === "string" ? attributes.segment : undefined;
  const charNodes = $createNestedChars(charAttr, viewOptions, textNode, segment);
  ...
  for (const node of charNodes) {
    if (!$insertNodeAtCharacterOffset(insertIndex, node, logger)) {
      allInserted = false;
      break;
    }
...
function $createNestedChars(
  ...
  if (Array.isArray(charAttr)) {
    ...
    if (
      $isCharNode(lastNode) &&
      lastNode.getMarker() === outerAttr.style &&
      $getState(lastNode, charIdState) === outerAttr.cid
    ) {
    ...
    if (
      $isCharNode(lastNode) &&
      lastNode.getMarker() === charAttr.style &&
      $getState(lastNode, charIdState) === charAttr.cid
    ) {
