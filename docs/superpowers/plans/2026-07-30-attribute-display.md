...
- New display only in `markerMode === "editable"` (Standard view) for char spans, verses, and UnknownNodes. The milestone attribute display already exists in visible mode; its completion (Task 8) applies to BOTH the editable and visible branches (TJ ruling, 2026-07-30). Hidden mode and Simple view must be behaviorally unchanged.
