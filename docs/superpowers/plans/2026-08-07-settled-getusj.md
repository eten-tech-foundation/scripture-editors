...
The cross-chapter flush no longer saves unsettled bytes. `handleEditorialUsjChange` schedules `editorRef.current?.getUsj() ?? usj` — the SETTLED, transient-excluded document, read at schedule time — rather than forwarding the raw `onUsjChange` payload, so `performDebouncedPdpSave`'s chapter-mismatch branch replays that already-settled CAPTURED snapshot when it fires: the editor has moved on to another chapter and cannot be read for the old one, but the snapshot it is replaying was never raw to begin with. Every save-scheduling site — `handleEditorialUsjChange`, the debounced-save capture, and the cross-chapter flush — now reads through `getUsj()` this way, so there is no remaining code path that can schedule an unsettled snapshot. That branch also no longer strips a palette literal (Task 13 deleted the strip), which is only reachable if a palette session somehow survives a chapter switch; navigation dismisses it, and `setTransientInput`'s exclusion at the schedule-time `getUsj()` read covers the bytes if it ever does not.
