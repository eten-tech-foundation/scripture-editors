...
**Qualification (2026-08-13):** this holds for a paste whose rebuild happens SYNCHRONOUSLY, within the paste's own commit (a terminated marker, or a marker-bearing line's Tier 2 rebuild via `armPasteRebuildDedup`) — one commit, one undo step. It does NOT hold for a paste whose literal text PENDS and only re-tokenizes on a LATER caret departure (e.g. a bare `//` with no terminator): the paste's own text insertion is one commit, and the departure-triggered settle is a deliberate SEPARATE commit — by design, not a bug — so undo restores the pre-settle literal on the first press and only reaches the pre-paste state on a second one. This is not paste-specific: the identical two-commit shape applies to a TYPED pending literal settled by departure, matching the project's documented undo-depth ruling that a settle "keeps its own [history] entry" whenever it actually settles something (`MarkerEditPlugin.tsx`). Measured and pinned for optbreak in `optbreakClipboardFidelity.test.tsx`'s "undo after a PLAIN-payload paste is TWO steps" pin.
...
| **Optbreak (`//`) clipboard fidelity** Copy characterization (`text/plain`/`text/html`), paste round trip across all three real-world payload shapes (plain-only, plain+html, plain+html+lexical), undo (one step for the lexical-flavor path; two steps for a plain-payload paste that pends until departure — S7's qualification), cut | `packages/platform/src/editor/markerEdit/optbreakClipboardFidelity.test.tsx`; the S2 fix itself lives in `libs/shared/src/nodes/features/UnknownNode.ts` (`excludeFromCopy`) |
