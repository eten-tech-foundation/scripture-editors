...
- **Paste inside an attribute display run — paste ≡ typing (live repro, 2026-08-11):** a selection sitting entirely inside attribute-display text — a char span's bare `|attrs` list, a milestone's attribute run, or a verse's `\va`/`\vp` value — is inserted exactly as the SAME characters typed at the SAME caret would be: `\r`/`\n` collapse to a single space (attribute values are single-line; there is no multi-line attribute byte shape), and NO chapter/book-id strip, NO positional NBSP mapping, and NO marker tokenization run against the pasted bytes — a pasted `\c 5` or `\p` there is literal value text, and a pasted NBSP passes through unchanged (matching `$displayWhitespaceTransform`'s own skip of attribute-tagged text). The existing attribute pend/settle machinery (`$textNodeTier2Transform`'s attribute-tagged early return, `$resolvePendingMarkers`) then re-tokenizes the displayed bytes back into node state identically whether they arrived by typing or paste. This SUSPENDS the same-namespace-flavor decline above (S2) for this one destination: live repro pasted `sid="things"` at the end of a char span's `|who="hi"` run while the clipboard also carried an internal same-editor `application/x-lexical-editor` payload, and the S2 decline handed the paste to Lexical's default rich-paste node insertion — which merged the attribute run, the char's own closing glyph, and the FOLLOWING paragraph's sibling text into one plain node, destroying the attribute display and the closing marker and leaving the pasted bytes loose in body content on save. A selection that sits only PARTLY inside attribute-display text (one end in, one end out) declines this special-casing and falls through to the ordinary external-paste path unchanged — see `$handlePasteForStandardView`'s doc comment (`whitespaceDisplay.plugin.utils.ts`) for the full mechanism and the mixed-selection rationale.
...
| **S3 (attribute-context paste)** Paste inside a char/milestone/verse attribute display run — paste ≡ typing | `packages/platform/src/editor/markerEdit/attributeContextPasteFidelity.test.tsx` — `"typed characterization (baseline)"`, `"paste ≡ typed (TJ's repro shape)"`, `"root cause: an internal same-editor copy on the clipboard must not corrupt the run"` (2026-08-11 live-repro regression), `"leading-space payload"`, `"replace-selection paste inside the attribute value"`, `"multi-line payload collapses to a single space"`, `"marker-bearing payload"`, `"CUT of a selection inside the attribute value"`, `"mixed selection"`, `"milestone attribute run paste"`, and `"verse \\va run paste"` describes |
