...
import { classify, mirrorRtl } from "./classifier.js";
...
describe("classify", () => {
  ...
  test("flags properties outside the spec lists as unknown and routes them to typography", () => {
    const unknowns = [
      "background-color",
      "border-left",
      "clear",
    ...
    for (const property of unknowns) {
      const result = classify({ property, value: "anything" });
      expect(result.bucket, `${property} bucket`).toBe("typography");
      expect(result.unknown, `${property} unknown flag`).toBe(true);
    }
  });
