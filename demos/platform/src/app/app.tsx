...
export default function App() {
  ...
  const customNodeOptions = useMemo<UsjNodeOptions>(
    () => ({
      noteCallerOnClick: (_event, _noteNodeKey, isCollapsed, getCaller, setCaller, getNoteOps) => {
        if (isCollapsed) {
  ...
  const handleNoteEditorSubmit = useCallback(() => {
    // TODO: Get USJ from note editor and apply update to main editor
    // const noteUsj = noteEditorRef.current?.getUsj();
    // marginalRef.current?.applyUpdate(/* ops derived from noteUsj */);
    console.log("Note editor submit - stubbed for now");
    noteEditorRef.current?.setUsj(emptyUsj);
    setIsNoteEditorVisible(false);
  }, []);
